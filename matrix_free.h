...
public:
  ...
  // L represents the delay in comuunication //edit by vd & skg
  int L = 1;
  ...
  /**
   * Same as above, but with std::function.     // edit by vd & skg
   */
  template < typename OutVector, typename InVector>
  void
  loop_cell_centric_CAA(
    ...
    const DataAccessOnFaces src_vector_face_access =
      DataAccessOnFaces::unspecified,
    int &indicator_part = 0) const;						// edit by vd & skg
...
//**************************************************** edit by vd & skg starts *****************************************************
  ...
  public:
    ...
    vector_compress_finish() override
    {
      ...
      if (!src_and_dst_are_same)                                                        // update by vd
        internal::reset_ghost_values(src, src_data_exchanger);
    }
  ...
  int &indicator_part) const
{
  ...
  task_info.loop(worker, this->communication, indicator_part);
}
